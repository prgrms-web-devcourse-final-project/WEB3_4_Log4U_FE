...
function randomTitle(): string {
  const prefixes = ['즐거운', '행복한', '신나는', '여유로운', '조용한', '설레는', '멋진'];
  const activities = ['여행', '일상', '카페 탐방', '맛집 탐방', '산책', '드라이브', '피크닉'];
...
function randomNickname(): string {
  const prefixes = ['행복한', '즐거운', '멋진', '귀여운', '신나는', '열정적인', '차분한'];
  const nouns = ['여행자', '사진사', '작가', '기록가', '탐험가', '미식가', '블로거'];
    ...
    export function Details(count: number): Diary.Detail[] {
      ...
      for (let i = 0; i < count; i++) {
        ...
        diaries.push({
          ...
          createdAt: createdAt,
          updatedAt: createdAt,
          thumbnailUrl: mediaList.length > 0 ? mediaList[0].url : randomImageUrl(),
...
const koreaRegions = {
  ...
  서울특별시: {
    ...
    dongmyun: {
      ...
      강서구: ['가양동', '개화동', '공항동', '등촌동', '방화동', '염창동', '화곡동'],
      마포구: ['망원동', '상암동', '서교동', '성산동', '신수동', '연남동', '합정동'],
    },
  },
  // 부산
  부산광역시: {
    ...
    dongmyun: {
      ...
      부산진구: ['부전동', '양정동', '전포동', '범천동', '범전동', '연지동', '초읍동'],
    },
  },
  // 인천
  인천광역시: {
  ...
  대구광역시: {
    sigungu: ['남구', '달서구', '동구', '북구', '서구', '수성구', '중구', '달성군'],
    dongmyun: {
      수성구: ['범어동', '만촌동', '황금동', '중동'],
    },
  },
...
function generateRandomSido(): keyof typeof koreaRegions {
  return sidoList[Math.floor(Math.random() * sidoList.length)] as keyof typeof koreaRegions;
}
